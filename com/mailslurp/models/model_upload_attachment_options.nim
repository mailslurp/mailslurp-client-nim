...
type UploadAttachmentOptions* = object
  ## Options for uploading files for attachments. When sending emails with the API that require attachments first upload each attachment. Then use the returned attachment ID in your `SendEmailOptions` when sending an email. This way you can use attachments multiple times once they have been uploaded.
  contentId*: string ## Optional contentId for file.
  contentType*: string ## Optional contentType for file. For instance `application/pdf`
  filename*: string ## Optional filename to save upload with. Will be the name that is shown in email clients
