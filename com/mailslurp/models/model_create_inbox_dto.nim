...
type CreateInboxDto* = object
  ...
  domainName*: string ## FQDN domain name for the domain you have verified. Will be appended with a randomly assigned recipient name. Use the `emailAddress` option instead to specify the full custom inbox.
  domainId*: string ## ID of custom domain to use for email address.
  name*: string ## Optional name of the inbox. Displayed in the dashboard for easier search and used as the sender name when sending emails.
  description*: string ## Optional description of the inbox for labelling purposes. Is shown in the dashboard and can be used with
  ...
  favourite*: bool ## Is the inbox a favorite. Marking an inbox as a favorite is typically done in the dashboard for quick access or filtering
  expiresIn*: int64 ## Number of milliseconds that inbox should exist for
  allowTeamAccess*: bool ## DEPRECATED (team access is always true). Grant team access to this inbox and the emails that belong to it for team members of your organization.
  ...
  virtualInbox*: bool ## Virtual inbox prevents any outbound emails from being sent. It creates sent email records but will never send real emails to recipients. Great for testing and faking email sending.
  useShortAddress*: bool ## Use a shorter email address under 31 characters
  prefix*: string ## Prefix to add before the email address for easier labelling or identification.
