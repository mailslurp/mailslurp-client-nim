...
import model_map
import model_object
import model_send_email_body_part

type SendStrategy* {.pure.} = enum
  SINGLEMESSAGE

type ValidateEmailAddresses* {.pure.} = enum
  VALIDATEFILTERREMOVEINVALID
  VALIDATEERRORIFINVALID
  NOVALIDATION
...
type SendEmailOptions* = object
  ...
  filterBouncedRecipients*: bool ## Filter recipients to remove any bounced recipients from to, bcc, and cc before sending
  validateEmailAddresses*: ValidateEmailAddresses ## Validate recipient email addresses before sending
  ignoreEmptyRecipients*: bool ## Ignore empty recipients after validation removes all recipients as invalid and fail silently
  isXAmpHtml*: bool ## Is content AMP4EMAIL compatible. If set will send as x-amp-html part.
  bodyParts*: seq[SendEmailBodyPart] ## Email body content parts for multipart mime message. Will override body.
...
func `%`*(v: ValidateEmailAddresses): JsonNode =
  let str = case v:
    of ValidateEmailAddresses.VALIDATEFILTERREMOVEINVALID: "VALIDATE_FILTER_REMOVE_INVALID"
    of ValidateEmailAddresses.VALIDATEERRORIFINVALID: "VALIDATE_ERROR_IF_INVALID"
    of ValidateEmailAddresses.NOVALIDATION: "NO_VALIDATION"

  JsonNode(kind: JString, str: str)

func `$`*(v: ValidateEmailAddresses): string =
  result = case v:
    of ValidateEmailAddresses.VALIDATEFILTERREMOVEINVALID: "VALIDATE_FILTER_REMOVE_INVALID"
    of ValidateEmailAddresses.VALIDATEERRORIFINVALID: "VALIDATE_ERROR_IF_INVALID"
    of ValidateEmailAddresses.NOVALIDATION: "NO_VALIDATION"
