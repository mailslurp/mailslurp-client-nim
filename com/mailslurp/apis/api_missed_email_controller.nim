...
import ../models/model_missed_email_dto
...
proc getMissedEmail*(httpClient: HttpClient, missedEmailId: string): (Option[MissedEmailDto], Response) =
  ## Get MissedEmail

  let response = httpClient.get(basepath & fmt"/missed-emails/{missedEmailId}")
  constructResult[MissedEmailDto](response)


proc restoreMissedEmails*(httpClient: HttpClient): Response =
  ## Restore missed emails
  httpClient.post(basepath & "/missed-emails/restore")


proc waitForNthMissedEmail*(httpClient: HttpClient, index: int, inboxId: string, timeout: int64, since: string, before: string): (Option[MissedEmailDto], Response) =
