...
proc waitForNthMissedEmail*(httpClient: HttpClient, inboxId: string, timeout: int64, index: int): (Option[MissedEmail], Response) =
  ...
  let response = httpClient.get(basepath & fmt"/missed-emails/waitForNthMissedEmail"fmt"/missed-emails/waitForNthMissedEmail" & "?" & query_for_api_call)
  constructResult[MissedEmail](response)
