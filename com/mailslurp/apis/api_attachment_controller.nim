...
import ../models/model_inline_object1
...
proc uploadAttachmentBytes*(httpClient: HttpClient, InlineObject1: InlineObject1, contentType: string, filename: string): (Option[seq[string]], Response) =
  ...
  let response = httpClient.post(basepath & "/attachments/bytes" & "?" & query_for_api_call, $(%InlineObject1))
  constructResult[seq[string]](response)


proc uploadMultipartForm*(httpClient: HttpClient, contentType: string, filename: string, x-filename: string, InlineObject: InlineObject): (Option[seq[string]], Response) =
