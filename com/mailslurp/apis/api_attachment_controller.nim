...
proc getAttachments*(httpClient: HttpClient, page: int, size: int, sort: string, fileNameFilter: string, since: string, before: string): (Option[PageAttachmentEntity], Response) =
...
proc uploadAttachment*(httpClient: HttpClient, UploadAttachmentOptions: UploadAttachmentOptions): (Option[seq[string]], Response) =
  ## Upload an attachment for sending using base64 file encoding. Returns an array whose first element is the ID of the uploaded attachment.
  httpClient.headers["Content-Type"] = "application/json"

  let response = httpClient.post(basepath & "/attachments", $(%UploadAttachmentOptions))
  constructResult[seq[string]](response)


proc uploadAttachmentBytes*(httpClient: HttpClient, contentType: string, contentType2: string, contentId: string, filename: string, filename2: string): (Option[seq[string]], Response) =
  ## Upload an attachment for sending using file byte stream input octet stream. Returns an array whose first element is the ID of the uploaded attachment.
  httpClient.headers["contentType"] = contentType
  httpClient.headers["filename"] = filename2
  let query_for_api_call = encodeQuery([
    ("contentType", $contentType2), # Optional contentType for file. For instance `application/pdf`
    ("contentId", $contentId), # Optional content ID (CID) to save upload with
    ("filename", $filename), # Optional filename to save upload with
  ])

  let response = httpClient.post(basepath & "/attachments/bytes" & "?" & query_for_api_call)
  constructResult[seq[string]](response)


proc uploadMultipartForm*(httpClient: HttpClient, contentId: string, contentType: string, filename: string, x-filename: string, InlineObject: InlineObject): (Option[seq[string]], Response) =
  ## Upload an attachment for sending using a Multipart Form request. Returns an array whose first element is the ID of the uploaded attachment.
  httpClient.headers["Content-Type"] = "application/json"
  let query_for_api_call = encodeQuery([
    ("contentId", $contentId), # Optional content ID of attachment
  ...
  let response = httpClient.post(basepath & "/attachments/multipart" & "?" & query_for_api_call, $(%InlineObject))
  constructResult[seq[string]](response)
