...
proc waitForWebhookResults*(httpClient: HttpClient, webhookId: string, expectedCount: int, timeout: int): (Option[seq[WebhookResultDto]], Response) =
  ## Wait for webhook results for a webhook
  let query_for_api_call = encodeQuery([
    ("expectedCount", $expectedCount), # Expected result count
    ("timeout", $timeout), # Max time to wait in milliseconds
  ])

  let response = httpClient.get(basepath & fmt"/webhooks/{webhookId}/wait" & "?" & query_for_api_call)
  constructResult[seq[WebhookResultDto]](response)
