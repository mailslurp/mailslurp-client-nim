...
proc submitForm*(httpClient: HttpClient, _to: string, _subject: string, _redirectTo: string, _emailAddress: string, _successMessage: string, _spamCheck: string, otherParameters: string): (Option[string], Response) =
  ...
  let response = httpClient.post(basepath & "/forms" & "?" & query_for_api_call)
