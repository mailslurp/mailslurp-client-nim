...
proc createNewEmailAddress*(httpClient: HttpClient, allowTeamAccess: bool, useDomainPool: bool, expiresAt: string, expiresIn: int64, emailAddress: string, inboxType: string, description: string, name: string, tags: seq[string], favourite: bool, virtualInbox: bool, useShortAddress: bool, domainName: string, domainId: string, prefix: string): (Option[InboxDto], Response) =
...
proc createRandomInbox*(httpClient: HttpClient, allowTeamAccess: bool, useDomainPool: bool, expiresAt: string, expiresIn: int64, emailAddress: string, inboxType: string, description: string, name: string, tags: seq[string], favourite: bool, virtualInbox: bool, useShortAddress: bool, domainName: string, domainId: string, prefix: string): (Option[InboxDto], Response) =
  ## Create new random inbox
  let query_for_api_call = encodeQuery([
    ("allowTeamAccess", $allowTeamAccess), # 
    ("useDomainPool", $useDomainPool), # 
...
proc sendEmailQuery*(httpClient: HttpClient, to: string, senderId: string, body: string, subject: string): Response =
  ## Send an email using query parameters
  let query_for_api_call = encodeQuery([
    ("senderId", $senderId), # ID of inbox to send from. If null an inbox will be created for sending
    ("to", $to), # Email address to send to
    ("body", $body), # Body of the email message. Supports HTML
    ("subject", $subject), # Subject line of the email
  ])
  httpClient.post(basepath & "/sendEmailQuery" & "?" & query_for_api_call)
