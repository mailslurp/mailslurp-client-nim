...
proc addContactsToGroup*(httpClient: HttpClient, groupId: string, UpdateGroupContacts: UpdateGroupContacts): (Option[GroupContactsDto], Response) =
  ## Add contacts to a group
  httpClient.headers["Content-Type"] = "application/json"

  let response = httpClient.put(basepath & fmt"/groups/{groupId}/contacts", $(%UpdateGroupContacts))
...
proc getAllGroups*(httpClient: HttpClient, page: int, size: int, sort: string, since: string, before: string): (Option[PageGroupProjection], Response) =
...
proc getGroupWithContactsPaginated*(httpClient: HttpClient, groupId: string, page: int, size: int, sort: string, since: string, before: string): (Option[PageContactProjection], Response) =
  ## 
  let query_for_api_call = encodeQuery([
...
proc removeContactsFromGroup*(httpClient: HttpClient, groupId: string, UpdateGroupContacts: UpdateGroupContacts): (Option[GroupContactsDto], Response) =
  ## Remove contacts from a group
  httpClient.headers["Content-Type"] = "application/json"

  let response = httpClient.delete(basepath & fmt"/groups/{groupId}/contacts", $(%UpdateGroupContacts))
  constructResult[GroupContactsDto](response)
