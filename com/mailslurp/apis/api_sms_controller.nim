...
import ../models/model_reply_for_sms
import ../models/model_sent_sms_dto
import ../models/model_sms_dto
import ../models/model_sms_reply_options
...
proc getReplyForSmsMessage*(httpClient: HttpClient, smsId: string): (Option[ReplyForSms], Response) =
  ## Get reply for an SMS message

  let response = httpClient.get(basepath & fmt"/sms/{smsId}/reply")
  constructResult[ReplyForSms](response)


proc getSmsMessage*(httpClient: HttpClient, smsId: string): (Option[SmsDto], Response) =
...
proc getUnreadSmsCount*(httpClient: HttpClient): (Option[UnreadCount], Response) =
  ## Get unread SMS count

  let response = httpClient.get(basepath & "/sms/unreadCount")
  constructResult[UnreadCount](response)


proc replyToSmsMessage*(httpClient: HttpClient, smsId: string, SmsReplyOptions: SmsReplyOptions): (Option[SentSmsDto], Response) =
  ## Send a reply to a received SMS message. Replies are sent from the receiving number.
  httpClient.headers["Content-Type"] = "application/json"

  let response = httpClient.post(basepath & fmt"/sms/{smsId}/reply", $(%SmsReplyOptions))
  constructResult[SentSmsDto](response)
