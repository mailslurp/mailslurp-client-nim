...
proc createInbox*(httpClient: HttpClient, emailAddress: string, tags: seq[string], name: string, description: string, useDomainPool: bool, favourite: bool, expiresAt: string, expiresIn: int64, allowTeamAccess: bool, inboxType: string, virtualInbox: bool, useShortAddress: bool, domainId: string, domainName: string, prefix: string): (Option[InboxDto], Response) =
  ...
  let query_for_api_call = encodeQuery([
    ...
    ("domainId", $domainId), # ID of custom domain to use for email address.
    ("domainName", $domainName), # FQDN domain name for the domain you have verified. Will be appended with a randomly assigned recipient name. Use the `emailAddress` option instead to specify the full custom inbox.
    ("prefix", $prefix), # Prefix to add before the email address for easier labelling or identification.
  ])
...
proc deleteAllInboxesByDescription*(httpClient: HttpClient, description: string): Response =
  ## Delete inboxes by description
  let query_for_api_call = encodeQuery([
    ("description", $description), # 
  ])
  httpClient.delete(basepath & "/inboxes/by-description" & "?" & query_for_api_call)


proc deleteAllInboxesByName*(httpClient: HttpClient, name: string): Response =
  ## Delete inboxes by name
  let query_for_api_call = encodeQuery([
    ("name", $name), # 
  ])
  httpClient.delete(basepath & "/inboxes/by-name" & "?" & query_for_api_call)


proc deleteAllInboxesByTag*(httpClient: HttpClient, tag: string): Response =
  ## Delete inboxes by tag
  let query_for_api_call = encodeQuery([
    ("tag", $tag), # 
  ])
  httpClient.delete(basepath & "/inboxes/by-tag" & "?" & query_for_api_call)
...
proc doesInboxExist*(httpClient: HttpClient, emailAddress: string, allowCatchAll: bool): (Option[InboxExistsDto], Response) =
  ## Does inbox exist
  let query_for_api_call = encodeQuery([
...
proc getAllInboxes*(httpClient: HttpClient, page: int, size: int, sort: string, favourite: bool, search: string, tag: string, teamAccess: bool, since: string, before: string, inboxType: string, inboxFunction: string, domainId: string): (Option[PageInboxProjection], Response) =
  ...
  let query_for_api_call = encodeQuery([
    ...
    ("inboxFunction", $inboxFunction), # Optional filter by inbox function
...
proc getAllInboxesOffsetPaginated*(httpClient: HttpClient, page: int, size: int, sort: string, favourite: bool, search: string, tag: string, teamAccess: bool, since: string, before: string, inboxType: string, inboxFunction: string, domainId: string): (Option[PageInboxProjection], Response) =
  ...
  let response = httpClient.get(basepath & "/inboxes/offset-paginated" & "?" & query_for_api_call)
  constructResult[PageInboxProjection](response)
...
proc getDeliveryStatusesByInboxId*(httpClient: HttpClient, inboxId: string, page: int, size: int, sort: string, since: string, before: string): (Option[PageDeliveryStatus], Response) {.deprecated.} =
  ## 
  let query_for_api_call = encodeQuery([
...
proc getInboxIds*(httpClient: HttpClient): (Option[InboxIdsResult], Response) {.deprecated.} =
  ## Get all inbox IDs
...
proc getInboxSentEmails*(httpClient: HttpClient, inboxId: string, page: int, size: int, sort: string, searchFilter: string, since: string, before: string): (Option[PageSentEmailProjection], Response) {.deprecated.} =
...
proc getInboxTags*(httpClient: HttpClient): (Option[seq[string]], Response) {.deprecated.} =
  ## Get inbox tags
...
proc getInboxes*(httpClient: HttpClient, size: int, sort: string, since: string, excludeCatchAllInboxes: bool, before: string): (Option[seq[InboxDto]], Response) {.deprecated.} =
  ...
  let query_for_api_call = encodeQuery([
    ...
    ("sort", $sort), # Optional createdAt sort direction ASC or DESC
    ("since", $since), # Optional filter by created after given date time
    ("excludeCatchAllInboxes", $excludeCatchAllInboxes), # Optional exclude catch all inboxes
    ("before", $before), # Optional filter by created before given date time
  ])
...
proc getOrganizationInboxes*(httpClient: HttpClient, page: int, size: int, sort: string, searchFilter: string, since: string, before: string): (Option[PageOrganizationInboxProjection], Response) {.deprecated.} =
