...
proc createInbox*(httpClient: HttpClient, A custom email address to use with the inbox. Defaults to null. When null MailSlurp will assign a random email address to the inbox such as `123@mailslurp.com`. If you use the `useDomainPool` option when the email address is null it will generate an email address with a more varied domain ending such as `123@mailslurp.info` or `123@mailslurp.biz`. When a custom email address is provided the address is split into a domain and the domain is queried against your user. If you have created the domain in the MailSlurp dashboard and verified it you can use any email address that ends with the domain. Note domain types must match the inbox type - so `SMTP` inboxes will only work with `SMTP` type domains. Avoid `SMTP` inboxes if you need to send emails as they can only receive. Send an email to this address and the inbox will receive and store it for you. To retrieve the email use the Inbox and Email Controller endpoints with the inbox ID.: string, Tags that inbox has been tagged with. Tags can be added to inboxes to group different inboxes within an account. You can also search for inboxes by tag in the dashboard UI.: seq[string], Optional name of the inbox. Displayed in the dashboard for easier search and used as the sender name when sending emails.: string, Optional description of the inbox for labelling purposes. Is shown in the dashboard and can be used with: string, Use the MailSlurp domain name pool with this inbox when creating the email address. Defaults to null. If enabled the inbox will be an email address with a domain randomly chosen from a list of the MailSlurp domains. This is useful when the default `@mailslurp.com` email addresses used with inboxes are blocked or considered spam by a provider or receiving service. When domain pool is enabled an email address will be generated ending in `@mailslurp.{world,info,xyz,...}` . This means a TLD is randomly selecting from a list of `.biz`, `.info`, `.xyz` etc to add variance to the generated email addresses. When null or false MailSlurp uses the default behavior of `@mailslurp.com` or custom email address provided by the emailAddress field. Note this feature is only available for `HTTP` inbox types.: bool, Is the inbox a favorite. Marking an inbox as a favorite is typically done in the dashboard for quick access or filtering: bool, Optional inbox expiration date. If null then this inbox is permanent and the emails in it won't be deleted. If an expiration date is provided or is required by your plan the inbox will be closed when the expiration time is reached. Expired inboxes still contain their emails but can no longer send or receive emails. An ExpiredInboxRecord is created when an inbox and the email address and inbox ID are recorded. The expiresAt property is a timestamp string in ISO DateTime Format yyyy-MM-dd'T'HH:mm:ss.SSSXXX.: string, Number of milliseconds that inbox should exist for: int64, DEPRECATED (team access is always true). Grant team access to this inbox and the emails that belong to it for team members of your organization.: bool, HTTP (default) or SMTP inbox type. HTTP inboxes are default and best solution for most cases. SMTP inboxes are more reliable for public inbound email consumption (but do not support sending emails). When using custom domains the domain type must match the inbox type. HTTP inboxes are processed by AWS SES while SMTP inboxes use a custom mail server running at `mx.mailslurp.com`.: string): (Option[InboxDto], Response) =
  ...
  let query_for_api_call = encodeQuery([
    ...
    ("Tags that inbox has been tagged with. Tags can be added to inboxes to group different inboxes within an account. You can also search for inboxes by tag in the dashboard UI.", $Tags that inbox has been tagged with. Tags can be added to inboxes to group different inboxes within an account. You can also search for inboxes by tag in the dashboard UI..join(",")), # 
    ("Optional name of the inbox. Displayed in the dashboard for easier search and used as the sender name when sending emails.", $Optional name of the inbox. Displayed in the dashboard for easier search and used as the sender name when sending emails.), # 
    ("Optional description of the inbox for labelling purposes. Is shown in the dashboard and can be used with", $Optional description of the inbox for labelling purposes. Is shown in the dashboard and can be used with), # 
    ...
    ("Is the inbox a favorite. Marking an inbox as a favorite is typically done in the dashboard for quick access or filtering", $Is the inbox a favorite. Marking an inbox as a favorite is typically done in the dashboard for quick access or filtering), # 
    ...
    ("Number of milliseconds that inbox should exist for", $Number of milliseconds that inbox should exist for), # 
    ("DEPRECATED (team access is always true). Grant team access to this inbox and the emails that belong to it for team members of your organization.", $DEPRECATED (team access is always true). Grant team access to this inbox and the emails that belong to it for team members of your organization.), # 
    ("HTTP (default) or SMTP inbox type. HTTP inboxes are default and best solution for most cases. SMTP inboxes are more reliable for public inbound email consumption (but do not support sending emails). When using custom domains the domain type must match the inbox type. HTTP inboxes are processed by AWS SES while SMTP inboxes use a custom mail server running at `mx.mailslurp.com`.", $HTTP (default) or SMTP inbox type. HTTP inboxes are default and best solution for most cases. SMTP inboxes are more reliable for public inbound email consumption (but do not support sending emails). When using custom domains the domain type must match the inbox type. HTTP inboxes are processed by AWS SES while SMTP inboxes use a custom mail server running at `mx.mailslurp.com`.), # 
  ])
...
proc getAllInboxes*(httpClient: HttpClient, page: int, size: int, sort: string, favourite: bool, search: string, tag: string, DEPRECATED. Optionally filter by team access.: bool, since: string, before: string): (Option[PageInboxProjection], Response) =
  ...
  let query_for_api_call = encodeQuery([
    ...
    ("DEPRECATED. Optionally filter by team access.", $DEPRECATED. Optionally filter by team access.), # 
...
proc getEmails*(httpClient: HttpClient, Id of inbox that emails belongs to: string, Alias for limit. Assessed first before assessing any passed limit.: int, limit: int, Sort the results by received date and direction ASC or DESC: string, retryTimeout: int64, delayTimeout: int64, minCount: int64, unreadOnly: bool, Exclude emails received after this ISO 8601 date time: string, Exclude emails received before this ISO 8601 date time: string): (Option[seq[EmailPreview]], Response) =
  ## Get emails in an Inbox. This method is not idempotent as it allows retries and waits if you want certain conditions to be met before returning. For simple listing and sorting of known emails use the email controller instead.
  let query_for_api_call = encodeQuery([
    ("Alias for limit. Assessed first before assessing any passed limit.", $Alias for limit. Assessed first before assessing any passed limit.), # 
    ...
    ("Sort the results by received date and direction ASC or DESC", $Sort the results by received date and direction ASC or DESC), # 
    ...
    ("Exclude emails received after this ISO 8601 date time", $Exclude emails received after this ISO 8601 date time), # 
    ("Exclude emails received before this ISO 8601 date time", $Exclude emails received before this ISO 8601 date time), # 
  ])
...
proc getInboxEmailCount*(httpClient: HttpClient, Id of inbox that emails belongs to: string): (Option[CountDto], Response) =
...
proc getInboxEmailsPaginated*(httpClient: HttpClient, Id of inbox that emails belongs to: string, page: int, size: int, sort: string, since: string, before: string): (Option[PageEmailPreview], Response) =
...
proc getInboxSentEmails*(httpClient: HttpClient, inboxId: string, page: int, Optional page size in inbox sent email list pagination: int, sort: string, searchFilter: string, since: string, before: string): (Option[PageSentEmailProjection], Response) =
  ## Get Inbox Sent Emails
  let query_for_api_call = encodeQuery([
    ("page", $page), # Optional page index in inbox sent email list pagination
    ("Optional page size in inbox sent email list pagination", $Optional page size in inbox sent email list pagination), # 
    ("sort", $sort), # Optional createdAt sort direction ASC or DESC
...
proc getInboxes*(httpClient: HttpClient, size: int, sort: string, since: string, before: string): (Option[seq[InboxDto]], Response) =
...
proc listInboxTrackingPixels*(httpClient: HttpClient, inboxId: string, page: int, Optional page size in inbox tracking pixel list pagination: int, sort: string, searchFilter: string, since: string, before: string): (Option[PageTrackingPixelProjection], Response) =
  ## List inbox tracking pixels
  let query_for_api_call = encodeQuery([
    ("page", $page), # Optional page index in inbox tracking pixel list pagination
    ("Optional page size in inbox tracking pixel list pagination", $Optional page size in inbox tracking pixel list pagination), # 
    ("sort", $sort), # Optional createdAt sort direction ASC or DESC
    ("searchFilter", $searchFilter), # Optional search filter
    ("since", $since), # Optional filter by created after given date time
    ("before", $before), # Optional filter by created before given date time
...
proc sendEmail*(httpClient: HttpClient, ID of the inbox you want to send the email from: string, SendEmailOptions: SendEmailOptions): Response =
  ## Send Email
  httpClient.headers["Content-Type"] = "application/json"
  httpClient.post(basepath & fmt"/inboxes/{inboxId}", $(%SendEmailOptions))


proc sendEmailAndConfirm*(httpClient: HttpClient, ID of the inbox you want to send the email from: string, SendEmailOptions: SendEmailOptions): (Option[SentEmailDto], Response) =
