...
proc getAllInboxes*(httpClient: HttpClient, before: string, favourite: bool, page: int, search: string, since: string, size: int, sort: string, tag: string, teamAccess: bool): (Option[PageInboxProjection], Response) =
...
proc getEmails*(httpClient: HttpClient, inboxId: string, before: string, delayTimeout: int64, limit: int, minCount: int64, retryTimeout: int64, since: string, size: int, sort: string, unreadOnly: bool): (Option[seq[EmailPreview]], Response) =
  ## Get emails in an Inbox. This method is not idempotent as it allows retries and waits if you want certain conditions to be met before returning. For simple listing and sorting of known emails use the email controller instead.
  let query_for_api_call = encodeQuery([
    ("before", $before), # Exclude emails received after this ISO 8601 date time
    ("delayTimeout", $delayTimeout), # delayTimeout
    ...
    ("unreadOnly", $unreadOnly), # unreadOnly
  ])
...
proc getInboxEmailsPaginated*(httpClient: HttpClient, inboxId: string, before: string, page: int, since: string, size: int, sort: string): (Option[PageEmailPreview], Response) =
...
proc getInboxSentEmails*(httpClient: HttpClient, inboxId: string, before: string, page: int, searchFilter: string, since: string, size: int, sort: string): (Option[PageSentEmailProjection], Response) =
...
proc getInboxes*(httpClient: HttpClient, before: string, since: string, size: int, sort: string): (Option[seq[Inbox]], Response) =
...
proc getOrganizationInboxes*(httpClient: HttpClient, before: string, page: int, searchFilter: string, since: string, size: int, sort: string): (Option[PageOrganizationInboxProjection], Response) =
...
proc listInboxRulesets*(httpClient: HttpClient, inboxId: string, before: string, page: int, searchFilter: string, since: string, size: int, sort: string): (Option[PageInboxRulesetDto], Response) =
  ## List inbox rulesets
  let query_for_api_call = encodeQuery([
    ("before", $before), # Optional filter by created before given date time
...
proc listInboxTrackingPixels*(httpClient: HttpClient, inboxId: string, before: string, page: int, searchFilter: string, since: string, size: int, sort: string): (Option[PageTrackingPixelProjection], Response) =
