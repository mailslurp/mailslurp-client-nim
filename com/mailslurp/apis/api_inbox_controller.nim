...
proc createInbox*(httpClient: HttpClient, allowTeamAccess: bool, description: string, emailAddress: string, expiresAt: string, expiresIn: int64, favourite: bool, name: string, tags: seq[string], useDomainPool: bool): (Option[Inbox], Response) =
  ...
  let query_for_api_call = encodeQuery([
    ...
    ("favourite", $favourite), # Is the inbox favorited. Favouriting inboxes is typically done in the dashboard for quick access or filtering
    ("name", $name), # Optional name of the inbox. Displayed in the dashboard for easier search
...
proc getInboxSentEmails*(httpClient: HttpClient, inboxId: string, page: int, size: int, sort: string): (Option[PageSentEmailProjection], Response) =
...
proc getOrganizationInboxes*(httpClient: HttpClient, page: int, size: int, sort: string): (Option[PageOrganizationInboxProjection], Response) =
