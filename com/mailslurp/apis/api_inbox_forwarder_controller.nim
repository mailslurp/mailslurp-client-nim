...
import ../models/model_inbox_forwarder_event_dto
...
proc getAllInboxForwarderEvents*(httpClient: HttpClient, page: int, size: int, sort: string): (Option[PageInboxForwarderEvents], Response) =
  ...
  let response = httpClient.get(basepath & "/forwarders/events" & "?" & query_for_api_call)
  constructResult[PageInboxForwarderEvents](response)


proc getForwarderEvent*(httpClient: HttpClient, eventId: string): (Option[InboxForwarderEventDto], Response) =
  ## Get a forwarder event

  let response = httpClient.get(basepath & fmt"/forwarders/events/{eventId}")
  constructResult[InboxForwarderEventDto](response)
...
proc getInboxForwarderEvent*(httpClient: HttpClient, id: string, eventId: string): (Option[InboxForwarderEventDto], Response) =
  ## Get an inbox forwarder event

  let response = httpClient.get(basepath & fmt"/forwarders/{id}/events/{eventId}"fmt"/forwarders/{id}/events/{eventId}")
  constructResult[InboxForwarderEventDto](response)
